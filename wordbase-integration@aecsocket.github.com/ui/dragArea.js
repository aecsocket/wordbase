...
export const DragArea = GObject.registerClass({
    ...
    Properties: {
        "width-request": GObject.ParamSpec.int(
            "width-request", null, null,
            GObject.ParamFlags.READWRITE,
            0, 0x7FFFFFFF, 180
        ),
        "height-request": GObject.ParamSpec.int(
            "height-request", null, null,
            GObject.ParamFlags.READWRITE,
            0, 0x7FFFFFFF, 100
        ),
    },
    ...
    vfunc_touch_event(event) {
        ...
        if (this._drag.grabbed_sequence.get_slot() === event.get_event_sequence().get_slot()) {
    ...
    _drag_begin(event) {
        ...
        const non_primary_button = Clutter.ModifierType.BUTTON2_MASK | Clutter.ModifierType.BUTTON3_MASK;
        /** @type {Mode} */
        let mode;
    ...
    _drag_motion(event) {
        ...
        if (this._drag.mode.type === "move") {
            ...
            let new_nw = [
                this._drag.mode.original_nw[0] + delta[0] * multiplier[0][0],
                this._drag.mode.original_nw[1] + delta[1] * multiplier[0][1],
            ];
            let new_se = [
                this._drag.mode.original_se[0] + delta[0] * multiplier[1][0],
                this._drag.mode.original_se[1] + delta[1] * multiplier[1][1],
            ];
