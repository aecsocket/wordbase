...
use {
    crate::{archive::OpenArchive, codec, storage},
    ...
    tracing::trace_span,
    wordbase_types::{DictionaryMeta, Record, RecordId, Term},
};
...
pub fn transaction<'a>(
    txn: &'a impl storage::ImportTransaction,
    ...
    impl<T: storage::ImportTransaction, C: codec::Codec> ImportTransaction for Transaction<'_, T, C> {
        fn batch(&self) -> Result<Box<dyn ImportBatch + '_>> {
            Ok(Box::new(Batch {
    ...
    impl<B: storage::ImportBatch, E: codec::Encoder> ImportBatch for Batch<'_, B, E> {
        fn insert_record_ref(&mut self, record: &Record) -> Result<RecordId> {
