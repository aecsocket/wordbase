...
impl Texthookers {
    pub(super) async fn new(
        ...
        let (new_url_tx, new_url_rx) = mpsc::channel(CHANNEL_BUF_CAP);
...
impl Engine {
    ...
    pub async fn set_texthooker_url(&self, url: impl Into<String>) -> Result<()> {
        ...
        _ = self.texthookers.new_url_tx.send(()).await;
        Ok(())
    }
}
