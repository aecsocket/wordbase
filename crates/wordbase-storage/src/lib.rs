...
use {
    ...
    wordbase_storage_api::{archive::OpenArchive, import::ImportProgress},
    wordbase_storage_backend::{
        backend::{Backend, ImportStorage, ImportTransaction, RecordRow},
        codec::Codec,
    },
};
pub use {
    wordbase_storage_api::archive,
    wordbase_storage_backend::{backend, codec},
    wordbase_storage_import as import,
};
...
pub async fn import<B: Backend, C: Codec + Default>(
    open_archive: &dyn OpenArchive,
    dict_dir: &Path,
) -> Result<()> {
    let import_ops = import::IMPORTERS
        .par_iter()
    ...
    let codec = C::default();
    let mut storage =
        B::create_import_storage(dict_dir).wrap_err("failed to create import storage")?;
    let storage_txn = storage
        .transaction()
        .wrap_err("failed to begin transaction")?;

    {
        let import_txn = wordbase_storage_api::import::transaction(&storage_txn, &codec);
...
impl<S: backend::LookupStorage, C: codec::Codec> Lookups<S, C> {
    pub fn lookup(&self, lemma: &str) -> Result<Vec<RecordRow>> {
        self.storage.lookup(|| self.codec.decoder(), lemma)
    }
}

pub async fn open<B: Backend, C: Codec>(
    dict_dir: &Path,
    codec: C,
) -> Result<(DictionaryManifest, Lookups<B::Lookups, C>)> {
    let manifest_path = dict_dir.join(MANIFEST_PATH);
    let manifest = async {
    ...
    let lookups = spawn_blocking(move || B::open(&dict_dir)).await??;
    Ok((
        manifest,
        Lookups {
            storage: lookups,
