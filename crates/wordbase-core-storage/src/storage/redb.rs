...
use {
    ...
    wordbase_types::{NoHeadwordOrReading, Record, RecordId, Term},
};
...
/// [`storage::Storage`] implementation which uses [`redb`] written in pure
/// Rust.
#[derive(Debug)]
pub struct Redb;

impl storage::Storage for Redb {
    type Lookups = LookupStorage;

    #[expect(refining_impl_trait, reason = "explicit refinement")]
    async fn create_import_storage(data_dir: &Path) -> Result<ImportTransaction> {
...
/// [`storage::ImportTransaction`] for [`Redb`].
#[derive(Debug)]
...
impl storage::ImportStorage for ImportTransaction {
    #[expect(refining_impl_trait, reason = "explicit refinement")]
...
impl<'txn> storage::ImportTransaction for ImportTables<'txn> {
    type Batch<'tbl>
        = ImportBatch<'txn, 'tbl>
    where
        Self: 'tbl;
...
/// [`storage::LookupStorage`] for [`Redb`].
#[derive(Debug)]
