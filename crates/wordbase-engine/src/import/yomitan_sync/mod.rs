...
async fn import_term(
    ...
    let record_id = insert_record(tx, source, &glossary, scratch).await?;
    insert_term_record(tx, &term, record_id).await?;
    Ok(())
}
...
async fn import_term_meta(
    ...
    match term_meta.data {
        schema::TermMetaData::Frequency(frequency) => {
            ...
            let term = Term::new(headword, reading).context("no headword or reading")?;

            let record_id = insert_record(tx, source, &record, scratch).await?;
            insert_term_record(tx, &term, record_id).await?;
        }
        schema::TermMetaData::Pitch(pitch) => {
            for (record, reading) in to_pitches_and_readings(pitch) {
                let term =
                    Term::new(headword.clone(), reading).context("no headword or reading")?;
                let record_id = insert_record(tx, source, &record, scratch).await?;
                insert_term_record(tx, &term, record_id).await?;
            }
        }
