...
pub static DEFAULT_THEME: LazyLock<Arc<Theme>> = LazyLock::new(|| {
    Arc::new(Theme {
        style: include_str!("default.css").to_string(),
    })
});
...
pub async fn watch_themes(data_dir: &Path) -> Result<notify::RecommendedWatcher> {
    ...
    match event.kind {
        notify::EventKind::Modify(ModifyKind::Name(RenameMode::Both)) => {
            ...
            if let Some(theme) = themes.remove(&from) {
                ...
                drop(themes);
                _ = EVENTS.send(AppEvent::ThemeRemoved(from));
                _ = EVENTS.send(AppEvent::ThemeAdded(theme));
            }
        }
        ...
        | notify::EventKind::Modify(ModifyKind::Name(RenameMode::From)) => {
            for path in event.paths {
                ...
                _ = EVENTS.send(AppEvent::ThemeRemoved(name));
            }
        }
