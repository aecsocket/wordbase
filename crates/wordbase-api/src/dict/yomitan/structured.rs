        ...
        const _: () = {
            impl fmt::Display for $T {
                fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
                    // 1. Convert to serde_json::Value to handle renames (e.g. kebab-case)
                    let value = serde_json::to_value(self).map_err(|_| fmt::Error)?;

                    // 2. If it's a string (enum variant), print it raw (no quotes).
                    //    Otherwise fallback to standard JSON representation.
