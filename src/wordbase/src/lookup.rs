...
impl Lookups {
        ...
        let query_ids = |table: &ReadOnlyMultimapTable<_, _>, err| {
            eyre::Ok(
                table
                    .get(lemma)
                    .wrap_err(err)?
                    .filter_map(|record_id| record_id.ok().map(|id| id.value())),
            )
        };
    ...
    pub fn lookup_lemma_rmp(&self, lemma: &str) -> Result<impl Iterator<Item = Result<Record>>> {
        Ok(self
            .lookup_record_ids(lemma)?
...
impl Lookup {
    pub fn unsorted(&self) -> impl Iterator<Item = RecordLookup<'_>> {
        self.records.iter().map(|record| RecordLookup {
            record: rkyv::access::<ArchivedRecord, Panic>(record.value()).unwrap(),
        })
    }
}
