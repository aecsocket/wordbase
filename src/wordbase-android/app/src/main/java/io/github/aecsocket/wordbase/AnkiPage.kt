...
fun AnkiPageApp(
    ...
    context.anki()?.let { anki ->
        val ankiPermission = rememberAnkiPermissionState()
        val ankiPermissionStatus = ankiPermission.status
        when (ankiPermissionStatus) {
            is PermissionStatus.Granted -> {
                val anki = anki.api(ankiPermissionStatus)
            ...
            is PermissionStatus.Denied -> AnkiNoPermission(ankiPermission)
        }
    } ?: AnkiNotInstalled()
}

@Composable
fun AnkiNotInstalled() {
    Column(
...
@Preview(showBackground = true)
@Composable
fun AnkiNotInstalledPreview() {
    AnkiNotInstalled()
}

@OptIn(ExperimentalPermissionsApi::class)
@Composable
fun AnkiNoPermission(permissionState: PermissionState) {
    Column(
        verticalArrangement = Arrangement.spacedBy(8.dp),
    ) {
        Text(
            text = stringResource(R.string.add_note_no_anki_permission)
        )
...
@Preview(showBackground = true)
@Composable
fun AnkiNoPermissionPreview() {
    AnkiNoPermission(rememberPermissionState(""))
}
