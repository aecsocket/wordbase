...
impl ImportKind for YomichanAudio {
    ...
    ) -> BoxFuture<'_, Result<(DictionaryMeta, ImportContinue)>> {
        Box::pin(async move {
    ...
    Terms: Iterator<Item = &'a Term>,
{
    ...
    // we need to flush after each insert, otherwise we get stuck while importing
    // NHK for some godforsaken unknown reason
